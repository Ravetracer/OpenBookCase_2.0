...
| GET | `/help` | `app_help` | Onboarding guide / tutorial (under Info menu). UI-icon-illustrated walkthrough of all features; **fully translated per-locale partials `static/help/{de,en,ru,nl,es,fr}.html.twig`** (one per supported UI locale; the wrapper falls back to English only for an unexpected locale). DaisyUI cards (not `prose`). **MAINTENANCE: any edit to the guide must be mirrored into all six locale partials.** |
| GET | `/changelog` | `app_changelog` | "What's new" page (under Info menu, after Guide). Release notes, newest first. Wrapper `static/changelog.html.twig` picks the per-locale partial `static/changelog/{de,en,ru,nl,es,fr}.html.twig` (one per supported UI locale; English fallback only for an unexpected locale), like `/help`. Each release is a `<section>` with a date heading + a `<ul>` of changes tagged with badges: `badge-success` = New, `badge-info` = Improved, `badge-warning` = Fixed (literal classes, no safelist needed). **MAINTENANCE: whenever a user-facing change ships, add a bullet to the current/most-recent release section in ALL SIX locale partials (`de,en,ru,nl,es,fr`)** (add a new dated `<section>` at the top for a new release day; localize the date heading + badge words per language). Menu label `nav.changelog` + page `static.changelog.{title,lead}` are in all 6 locales; the entry text lives in the six per-locale partials. |
| GET | `/developers` | `app_developers` | **"For developers"** API docs page (under Info menu, after What's-new). Same wrapper+per-locale-partial pattern as `/help`+`/changelog`: `static/developers.html.twig` picks `static/developers/{de,en,ru,nl,es,fr}.html.twig` (one per supported UI locale; English fallback only for an unexpected locale). Documents the OAuth2 flows (Authorization Code + PKCE for public/mobile clients, client secret for confidential), the `/api/v1` endpoint reference table, scopes, rate limits, error codes, and runnable `curl` examples (`{% verbatim %}`-wrapped code blocks); reuses `static/_data_terms.html.twig` for the ODbL terms. Nav label `nav.developers` + `static.developers.title` in all 6 locales; body text lives in the six per-locale partials. **MAINTENANCE: any edit to the docs must be mirrored into all six locale partials.** |
...
| GET | `/api/bookcase/export` | `api_bookcase_export` | Open-data dump of every entry (location + contact only; no images/ratings). **Streamed** one row at a time with `EntityManager::clear()` every 500 — the dataset is ~56k rows, so the old build-everything-then-encode approach hit the PHP memory limit (HTTP 500). Caretakers/opening times come from two bulk lookup maps keyed by ULID (not lazy-loaded). `?gzip=1` streams an on-the-fly gzip download (`.json.gz`, `application/gzip`) via incremental `deflate_add`. Linked from `/list` (dropdown: compressed = recommended, or plain). Declared **before** `/{bookcase}` so the literal wins |
...
- The original PHPMyAdmin bulk-import command (`app:import-phpmyadmin-json`) was **removed** — the legacy database is gone and this project's DB is now the source of truth. Legacy *user* accounts still live in the DB and migrate on first login (above); only the one-off import tooling is gone.
