...
```
src/
├── Command/              # CLI commands (ImportPhpMyAdminJsonCommand, ImportOsmCommand, ImportLesestundenCommand, GenerateShortCodesCommand, FixHtmlEntitiesCommand, SendSystemMessageCommand)
...
- `app:import-lesestunden` (`ImportLesestundenCommand`) imports **only NEW** bookcases from a third-party `var/lesestunden.json` export (that site originally scraped our data, so most entries are duplicates). Dedup is by **coordinate proximity** via an in-memory spatial grid index (existing coords + ones added during the run), confirmed with an exact haversine ≤ `--threshold` metres (default 50). **Idempotent** — a just-imported entry matches itself next run, so re-run anytime to pull in what's new. Flags: `-f/--file`, `-t/--threshold` (metres), `--dry-run` (report counts, write nothing). Imported entries are plain bookcases (`entry_type=bookcase`, `map_symbol=standard`, `active`), with address/webpage/opening-time (24-7 auto-detect) and a caretaker from the `contact` field; `legacy_id` is left NULL (not our legacy system). **Anti-spam:** source rows whose `title` contains a URL (`http(s)://` or `www.`) are skipped (reported as "Spam … skipped").
